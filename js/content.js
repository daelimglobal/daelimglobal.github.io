/* ================================================
   content.js — 관리자가 저장한 콘텐츠를 불러와
   메인 홈페이지에 반영합니다.
   관리자 패널에서 저장 → localStorage → 이 파일이 읽음
   ================================================ */
  ...
  social: [
    {
      id:1,
      tag:'주거환경 개선',
      title:'독거노인 주거환경 개선 사업',
      desc:'취약계층 어르신들의 낡은 주거환경을 개선하여 안전하고 따뜻한 삶을 지원합니다.',
    ...
    },
    {
      id:2,
      tag:'사랑의 집짓기',
      title:'저소득층 주거 개보수 지원',
      desc:'저소득층 가정의 도배·장판 교체 및 노후 시설 개선을 통해 더 나은 삶의 환경을 만들어 드립니다.',
    ...
    },
    {
      id:3,
      tag:'지역사회 봉사',
      title:'지역아동센터 환경개선 활동',
      desc:'아이들이 안전하게 성장할 수 있도록 지역아동센터의 시설을 개선하고 봉사활동을 진행합니다.',
      img:'https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=600&q=80&auto=format&fit=crop',
      date:'2024.09'
    },
  ],
  videos: [
    { id:1, title:'베토벤 40평형 시공 현장', youtubeId:'' },
    { id:2, title:'대림글로벌 회사 소개', youtubeId:'' },
    { id:3, title:'준공 완료 인터뷰', youtubeId:'' },
  ],
  info: {
    phone:        '1668-3603',
    hours:        '평일 09:00 - 18:00',
    address:      '전국 어디서나 상담 가능',
    kakao:        '#',
    copyright:    '© 2024 (주)대림글로벌 DAELIM GLOBAL. All rights reserved.',
...
function load(key, fallback) {
  try {
    const v = localStorage.getItem('dg_' + key);
...
function renderPortfolio() {
  ...
  if (!grid) return;
  const items = load('portfolio', DEFAULTS.portfolio);
...
function renderSocial() {
  ...
  if (!grid) return;
  const items = load('social', DEFAULTS.social);
...
function renderVideos() {
  ...
  if (!grid) return;
  const items = load('videos', DEFAULTS.videos).filter(v => v.youtubeId && v.youtubeId.trim());
  if (!items.length) {
...
function applyInfo() {
  const info = load('info', DEFAULTS.info);
...
function applyCustomText() {
  ...
  const fields = [
    ...
    'stat1Label','stat2Label',
  ];
  fields.forEach(id => {
    if (ct[id]) {
      const el = document.getElementById(id);
      if (el) el.innerHTML = ct[id];
    }
  });
}
